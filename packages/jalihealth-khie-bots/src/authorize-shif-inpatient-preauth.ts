...
import type { AuthorizeShifInpatientPreauthInput, KhieWorkflowDependencies } from './types';

export function createAuthorizeShifInpatientPreauthHandler(dependencies: KhieWorkflowDependencies = {}) {
  return async function handler(medplum: MedplumClient, event: BotEvent<AuthorizeShifInpatientPreauthInput>) {
    ...
    if (
      getPayerCaseInput(payerCase.task, 'KHIE payment mechanism') !== 'FEE_FOR_SERVICE' ||
      getPayerCaseInput(payerCase.task, 'KHIE preauthorization path') !== 'elective'
    ) {
      throw new Error('Pre-visit authorization is only available for elective SHIF inpatient FFS cases');
    }
...
export const handler = createAuthorizeShifInpatientPreauthHandler();